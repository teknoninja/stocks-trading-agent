// TradingView sidebar bot — injected by tradingview_bot/browser.py
// Talks to the local FastAPI server (http://127.0.0.1:{PORT}).
(function () {
  // Injected as a Playwright init script — may run before <body> exists.
  ...
  panel.innerHTML = `
    ...
    <div style="padding:4px 12px 8px;color:#6b7280;font-size:10px;text-align:center">Educational only — not financial advice ·
      <a href="http://127.0.0.1:__PORT__/performance" target="_blank" style="color:#60a5fa">performance</a> ·
      <a href="http://127.0.0.1:__PORT__/watchlists" target="_blank" style="color:#60a5fa">watchlists</a></div>`;
  ...
  async function refreshAnalysis(symbol) {
    ...
    try {
      ...
      $("tvb-conf").textContent = "Local bot server unreachable on port __PORT__";
    } finally {
      analyzing = false;
    }
  }
  ...
  async function send() {
    ...
    try {
      const r = await fetch(`${API}/ask`, {
        ...
        body: JSON.stringify({ symbol: currentSymbol, question: q }),
      });
      const d = await r.json();
      pending.textContent = d.answer;
    } catch (e) {
      pending.textContent = "Server unreachable — is run_tradingview_bot.py still running?";
    }
  }
  ...
  function refreshWatch(sym) {
    fetch(`${API}/watchlist/status?symbol=${encodeURIComponent(sym)}`)
      .then((r) => r.json()).then(paintWatch).catch(() => paintWatch(null));
  }
  ...
  fetch(`${API}/autotrade`).then((r) => r.json())
    .then((s) => { autoState = s.configured ? s.enabled : null; paintAuto(); })
    .catch(() => paintAuto());
  $("tvb-auto").onclick = async () => {
    ...
    try {
      const r = await fetch(`${API}/autotrade`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ on: !autoState }),
      });
      const d = await r.json();
      if (d.ok) {
